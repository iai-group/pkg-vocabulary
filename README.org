...
The PKG Vocabulary is defined by a set of SHACL shapes patterns definition over largely well-established vocabularies:
 ...
 - Complex values are recommended expressed using SKOS' Concepts following the ~pkg:ConceptShape~ pattern.
...
.
  ...
  ## FURTHER ANALYSIS, the statement may be analysed in more detail, if possible, e.g., 
  ## from the statement we can derive I's negative preference towards Tom Cruise.
  my:I wi:preference [
